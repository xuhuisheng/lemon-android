...
public class MsgAsyncTask extends AsyncTask<ListView, Void, Void> {
    ...
    public MsgAsyncTask(String urlText, Activity activity, String sessionId) {
    ...
    public void updateData() {
        ...
        try {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
				...
				map.put("msgId", (String) jsonObject.get("id"));
                map.put("username", (String) jsonObject.get("senderUsername"));
                map.put("content", (String) jsonObject.get("content"));
				map.put("messageTime", "2016-02-24");
                list.add(map);
            }

            // 生成适配器，数组===》ListItem
            SimpleAdapter simpleAdapter = new SimpleAdapter(activity, list,
                    R.layout.tt_item_chat,
                    new String[] { "username", "content", "messageTime" }, new int[] {
                            R.id.shop_name, R.id.message_body, R.id.message_time });
            // 添加并且显示
